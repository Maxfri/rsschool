...
export class Garage extends BaseComponent {
  ...
  async renderGarage(pageNumber: number) {
    ...
    garage.innerHTML = `<h1>Garage (${carsCount})</h1>
        ...
        <ul class="garage">
        ${cars.map((car: Car) => `<li>${this.renderCar(car)}</li>`).join('')}
        </ul>`;
  ...
  renderCar = (car: Car) => `<div class="car-wrapper">
  <div class="car-button">
    <button class="remove">remove car</button>
    <button class="edit">edit car</button>
  </div>
