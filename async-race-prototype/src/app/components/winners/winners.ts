import { getWinners, Sort, Order } from '../../../api/winners/winners';
...
const FIRST_PAGE = 1;
const MAX_CARS_IN_PAGE = 10;
const TITLE_WINNERS_PAGE = '.winners-title';
const SUBTITLE_WINNERS_PAGE = '.winners-subtitle';
const LIST_WINNERS_ON_PAGE = '.winners-list';
...
export class Winners extends BaseComponent {
  ...
  constructor() {
    ...
    next?.addEventListener('click', () => {
      this.page++;
      this.renderWinners(this.page, <Sort>store.sortdBy, <Order>store.sortOrder);
    });
  ...
  render(): HTMLElement {
    this.element.innerHTML = `<h1 class="winners-title">Winners</h1>
      <h2 class="winners-subtitle">Page</h2>
      <section class="winners-table">
        <header class="winners-table-header">
          <div class="col">№</div>
          <div class="col">Image of the car</div>
          <div class="col sort-name sort" data-order="ASC">Name of the car</div>
          <div class="col sort-wins sort" data-order="ASC">Wins number</div>
          <div class="col sort-time sort" data-order="ASC">Best time in seconds</div>
        </header>
  ...
  async renderWinners(pageNumber: number, sort: Sort, order: Order): Promise<void> {
    this.winnersList = [];
    const result = (await getWinners(pageNumber, MAX_CARS_IN_PAGE, sort, order));
    const winners = result.items;
    const winnersCount = <string>result.count;

    const winnersTable = <HTMLElement>document.querySelector(LIST_WINNERS_ON_PAGE);
    winnersTable.innerHTML = '';
    const title = <HTMLElement>document.querySelector(TITLE_WINNERS_PAGE);
    const subtitle = <HTMLElement>document.querySelector(SUBTITLE_WINNERS_PAGE);
  ...
  async sorting(element: HTMLElement, sort: Sort): Promise<void> {
    let order: Order = <Order>element.getAttribute('data-order');
